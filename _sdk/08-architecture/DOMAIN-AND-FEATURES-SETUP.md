...
## THE SHOWCASE BUNDLE PATTERN

Showcase bundles are Features used for **component demonstrations** in admin panels. They hold throwaway demo state to make VR showcases interactive.

```
src/features/admin/showcase-bundle/
├── index.tsx              ← Demo features (throwaway state)
├── showcase-bundle.css    ← Guide styling (ft-showcasetabs-* prefix)
└── (no _tabs needed)
```
...
export function RadiosShowcase() {
  const [value, setValue] = useState('option1');  // Throwaway state

  return (
    <Card.standard title="Radios Showcase">
      <Input.radio
        value={value}
        onChange={setValue}
...
**VR Guide CSS Pattern:**
```css
/* ft-showcasetabs-guide - 2-column layout for documentation */
.ft-showcasetabs-guide {
...
.ft-showcasetabs-guide h3 {
  font-size: var(--font-size-md, 16px);
  font-weight: var(--font-weight-semibold, 600);
}

.ft-showcasetabs-guide-code {
  ...
  font-size: var(--font-size-md, 16px);
  text-align: center;
  color: var(--brand-primary, #f40);
  margin: var(--space-xs, 4px) 0 var(--space-sm, 8px) 0;
}
```

**Key Differences:**
- **NOT production logic** - Just demo state for sandboxing VRs
- **Real pages wire VRs to FUSE** - Showcases wire VRs to `useState`
- **Purpose:** Make component showcases interactive without touching FUSE
- **Grid layouts:** Use `ft-showcasetabs-grid-*` classes for demo grids
- **Documentation:** Use `ft-showcasetabs-guide` for VR implementation guides

**When to use:**
- Admin panel VR showcase pages
- Component documentation tabs
- Interactive VR sandboxes

**When NOT to use:**
- Production features (use real Features with FUSE)
- Anything that touches database or business logic
...
- **ft-showcasetabs-** prefix for Showcase CSS: `ft-showcasetabs-guide`, `ft-showcasetabs-grid-5`
...
| VR showcase/sandbox | `features/admin/showcase-bundle/` | Demo features with useState |
