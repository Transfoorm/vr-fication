 ...
 *
 * "Let's Go!" click → confetti + confetti sound + connected sound + dismiss
 */
export function EmailConnectedModal({ emailAddress, onDismiss }: EmailConnectedModalProps) {
  // Prime sounds on mount for instant playback
  useEffect(() => {
    sounds.prime();
  }, []);

  // Let's Go button: confetti + both sounds + dismiss
  const handleLetsGo = () => {
    fireConfetti();
    sounds.confetti();
    sounds.connected();
  ...
  return (
    <>
      <div className="ft-email__modal-backdrop ft-email__modal-backdrop--celebration" />
      <div className="ft-email__modal ft-email__modal--celebration">
        ...
        <Button.fire onClick={handleLetsGo} icon={<Sparkles size={16} />}>
          Let&apos;s Go!
        </Button.fire>
      </div>
    </>
